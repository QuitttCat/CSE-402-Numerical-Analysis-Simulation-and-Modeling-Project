"""
beh2_surface_gif.py -- render a saved BeH2 surface scan as an animated GIF:
the 3D energy surface rises out of the grid one VQE point at a time, in the
order the scan actually visited them, with the molecule deforming alongside;
then the finished surface turns once on the spot.

RUNS NO CHEMISTRY. This reads the ``beh2_surface_scan.json`` that
``beh2_ground_state_estimation.py --coordinate surface`` saved and never
calls VQE. It takes about a minute, nearly all of it matplotlib drawing
~200 frames, and can be re-run freely.

The output is what the README's "A Scan in Action" section shows
(``assets/beh2_surface_scan_3d.gif``). Re-run this after re-scanning the
surface -- a finer grid, a different active space -- and the README picture
updates itself.

WHY A SEPARATE SCRIPT
----------------------
``--load beh2_surface_scan.json`` on the main script replays the scan live as
a 2D heat map filling in, then pops the finished 3D surface as a static
figure. That is right for sitting at the keyboard -- re-rendering a
``plot_surface`` per point is slow and a half-empty 3D surface is hard to read
interactively. But for a picture in a README the trade-off flips: nobody is
waiting on the frames, and watching the *surface itself* grow is the more
striking view of the same 110 numbers. So this script rebuilds the 3D view
offscreen, frame by frame, and never touches the interactive viewer.

It borrows the molecule-drawing helpers and the atom colours straight from
``beh2_ground_state_estimation`` (import only), so the Geometry panel here is
pixel-for-pixel the one the live window draws.

WHAT THE ANIMATION SHOWS
-------------------------
  grow   One frame per scanned geometry, in serpentine order (the warm-start
         order the scan ran in -- see ``serpentine()`` in the main script).
         A black dot lands on every solved point and a white diamond marks
         the newest one. ``plot_surface`` needs all four corners of a cell,
         so the sheet lags the dots by one row: single points appear first,
         the surface knits between them a row later. The z range and the
         colour scale are fixed to the *final* data from frame one, so
         nothing rescales while it fills.

  spin   One full turn about the vertical axis, ending back at the starting
         view so the loop is seamless. The Geometry panel parks on the
         minimum-energy geometry for the whole spin.
...
REQUIREMENTS
-------------
Pillow, for writing the GIF. It is already installed as a dependency of
matplotlib, so nothing extra is needed beyond ``requirements.txt``.

USAGE
------
  DEFAULTS -- beh2_surface_scan.json next to this script -> assets/beh2_surface_scan_3d.gif
    python experiment/beh2/beh2_surface_gif.py

  A DIFFERENT SCAN, OR A DIFFERENT OUTPUT
    python experiment/beh2/beh2_surface_gif.py --scan fine_surface.json
    python experiment/beh2/beh2_surface_gif.py --out ~/slides/beh2_surface.gif

  TUNING
    --dpi 72             render resolution (72 -> ~890 px wide, about 3.5 MB)
    --frame-ms 70        time per scan point during the grow phase
    --spin-frames 90     frames in the final rotation; 0 skips the spin
    --elev 28 --azim -135   starting camera, same as the main script's 3D view
"""
...
import beh2_ground_state_estimation as beh2  # noqa: E402  -- import only, runs nothing
from scan_io import display_path, load_scan  # noqa: E402

DEFAULT_SCAN = "beh2_surface_scan.json"
DEFAULT_OUT = HERE.parents[1] / "assets" / "beh2_surface_scan_3d.gif"
...
def render(scan_path: Path, out: Path, dpi: int, frame_ms: int, spin_frames: int,
    ...
    bonds = np.asarray(metadata["bond_values"], dtype=float)
    angles = np.asarray(metadata["angle_values"], dtype=float)
    grid_bond, grid_angle = np.meshgrid(bonds, angles)
    Z = np.full(grid_angle.shape, np.nan)          # [angle, bond], like the viewer
    ...
    # -- figure: 3D surface left, molecule right -----------------------
    fig = plt.figure(figsize=(12.5, 5.8), dpi=dpi)
    ax = fig.add_axes([-0.02, 0.0, 0.66, 1.0], projection="3d", computed_zorder=False)
    ax.set_xlim(bonds[0], bonds[-1])
    ax.set_ylim(angles[0], angles[-1])
    ax.set_zlim(zmin - zpad, zmax + zpad)
    ax.set_xlabel("r(Be–H)  (Å)", labelpad=8)
    ax.set_ylabel("H–Be–H angle  (deg)", labelpad=8)
    ax.set_zlabel("Energy  (Ha)", labelpad=10)
    ax.set_title("BeH$_2$ ground-state surface (VQE)", y=0.96)
    # Wider along the bond axis than the angle axis, and squat in z, so the
    # trough reads as a valley rather than a cliff.
    ax.set_box_aspect((1.35, 1.0, 0.75), zoom=1.02)
    ...
    ax_mol = fig.add_axes([0.715, 0.17, 0.27, 0.66])
    span = float(bonds[-1]) + 0.5
    patches, bond_lines, label = beh2._build_molecule_panel(ax_mol, span)
    ...
    for n, row in enumerate(rows, start=1):
        j = int(np.argmin(np.abs(bonds - row["bond"])))    # nearest, not ==: float round-trip via JSON
        i = int(np.argmin(np.abs(angles - row["angle"])))
        Z[i, j] = row["vqe"]
        done_x.append(row["bond"]); done_y.append(row["angle"]); done_z.append(row["vqe"])
        dots._offsets3d = (done_x, done_y, done_z)
        marker.set_data([row["bond"]], [row["angle"]])
    ...
    marker.set_data([best["bond"]], [best["angle"]])
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Animate a saved BeH2 surface scan as a growing, then rotating, 3D GIF. "
                    "Runs no chemistry.",
    )
    ...
    parser.add_argument("--out", default=str(DEFAULT_OUT), metavar="FILE",
                        help="GIF to write (default: assets/beh2_surface_scan_3d.gif at the repo root)")
    parser.add_argument("--dpi", type=int, default=72,
                        help="render resolution; 72 gives ~890 px wide (default: 72)")
    parser.add_argument("--frame-ms", type=int, default=70,
                        help="milliseconds per scan point while the surface grows (default: 70)")
    parser.add_argument("--spin-frames", type=int, default=90,
                        help="frames in the closing rotation; 0 to skip it (default: 90)")
    parser.add_argument("--elev", type=float, default=28.0, help="camera elevation (default: 28)")
    parser.add_argument("--azim", type=float, default=-135.0,
                        help="starting camera azimuth; the spin returns here (default: -135)")
