"""
n2_polynomial_interpolation.py -- reconstruct N2's dissociation curve from a
...
``experiment/n2/n2_ground_state_estimation.py`` already computed and saved,
and does pure numerical analysis on it. Seconds, not hours.
...
For N2 that failure mode lands in a particularly bad spot. The default scan
is nine points, so the full interpolating polynomial is degree 8, and the
region where the oscillation is worst -- the ends -- is exactly where N2's
own physics is least forgiving: a hard, narrow repulsive wall on the short-
bond side (the triple bond makes N2 stiffer than H2 or O2 at the same
fractional compression) and the dissociation plateau on the long-bond side,
where the RHF-based curve is already behaving badly for reasons that have
nothing to do with interpolation (see the module docstring of
``experiment/n2/n2_ground_state_estimation.py``). Stacking a spurious
polynomial wobble on top of that plateau can invent a fake second minimum
that was never in the underlying chemistry, let alone the data.

Compare this against ``../n2_spline/``, which fits low-degree cubics
...
carry no information; the leave-one-out error is what actually distinguishes
a good reconstruction from a bad one. The script sweeps every usable degree
and reports the error for each, which is what makes the degradation visible:
...
conditioned. Fitting a degree-8 polynomial in raw Angstroms is numerically
nasty, and without that rescaling some of the oscillation you would see
would be floating-point noise rather than the genuine Runge effect this
script is trying to show. Getting that distinction right matters when the
whole point is to attribute the failure correctly.

OUTPUT
-------
  n2_polynomial_interp.json / .csv    per-node leave-one-out errors at the
                                      chosen degree, plus summary metrics
  n2_polynomial_degrees.csv           leave-one-out error for every degree
  n2_polynomial_dense.csv             the reconstructed curve, densely sampled
  n2_polynomial_curve.png             nodes, the polynomial, and linear for
                                      comparison
  n2_polynomial_error.png             per-node error, polynomial vs linear
  n2_polynomial_degree_sweep.png      error against degree -- the Runge story
...
experiment/n2/ already saved.

  DEFAULT -- finds n2_scan.json (or .csv) in experiment/n2/
    python experiment/n2_polynomial/n2_polynomial_interpolation.py

  A PARTICULAR SCAN -- .json preferred, .csv accepted
    python experiment/n2_polynomial/n2_polynomial_interpolation.py --scan n2_scan.json
    python experiment/n2_polynomial/n2_polynomial_interpolation.py --scan n2_scan.csv
    python experiment/n2_polynomial/n2_polynomial_interpolation.py --scan fine_scan.json

  WHICH ENERGY COLUMN -- 'vqe' by default. Interpolating 'exact' instead
  separates coarse-sampling error from VQE optimizer noise.
    python experiment/n2_polynomial/n2_polynomial_interpolation.py --column vqe
    python experiment/n2_polynomial/n2_polynomial_interpolation.py --column exact
    python experiment/n2_polynomial/n2_polynomial_interpolation.py --column hf

  POLYNOMIAL DEGREE -- the default is 8 here (n_nodes-1 for the 9-point scan),
  which is exactly the one degree leave-one-out cannot validate. The sweep
  covers every degree that can be.
    python experiment/n2_polynomial/n2_polynomial_interpolation.py --degree 6
    python experiment/n2_polynomial/n2_polynomial_interpolation.py --degree 3
...
DEFAULT_SCAN_NAMES = ["n2_scan.json", "n2_scan.csv"]


def find_scan(explicit: str | None, here: Path) -> Path:
    """Locate the saved N2 scan to interpolate. See the n2_spline twin."""
    n2_dir = here.parents[0] / "n2"
    if explicit is not None:
        candidates = [Path(explicit), here / explicit, n2_dir / explicit]
    else:
        candidates = [n2_dir / name for name in DEFAULT_SCAN_NAMES]
    ...
    raise SystemExit(
        f"no saved N2 scan found. Looked in:\n  {looked}\n\n"
        f"This script interpolates an existing scan; it does not run any chemistry.\n"
        f"Produce one first with:\n"
        f"  python experiment/n2/n2_ground_state_estimation.py"
    )


def _rows_from_csv(path: Path) -> tuple:
    """Read an N2 scan from its ``.csv``. Metadata is inferred; see the scan script."""
    ...
    if missing:
        raise SystemExit(f"{display_path(path)} lacks column(s) {sorted(missing)} -- "
                         f"not an N2 scan CSV.")
    return {"molecule": "N2"}, rows
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Reconstruct N2's dissociation curve from a coarse VQE scan with a "
    ...
    parser.add_argument("--scan", type=str, default=None, metavar="FILE",
                        help="the saved N2 scan to interpolate (default: look for "
                             "n2_scan.json in experiment/n2/)")
    ...
    parser.add_argument("--save", type=str, default=None, metavar="FILE",
                        help="where to save the results (default: "
                             "n2_polynomial_interp.json/.csv next to this script)")
    ...
    if n < 3:
        raise SystemExit(
            ...
            f"about polynomial degree. Re-run the N2 scan with a finer --step."
        )
    ...
    print(f"  {metadata.get('molecule', 'N2')} "
    ...
    print("  compare with the cubic spline: python experiment/n2_spline/n2_spline_interpolation.py")
    print()
    ...
    ax.set_xlabel("N-N bond length (Angstrom)")
    ax.set_ylabel(f"Energy (Hartree)  [{args.column}]")
    ax.set_title(f"N$_2$ curve reconstructed by a degree-{degree} polynomial\n"
    ...
    ax_err.set_xlabel("node left out (N-N bond length, Angstrom)")
    ...
    # -- save -----------------------------------------------------------
    if not args.no_save:
        target = resolve_target(args.save, here, "n2_polynomial_interp")
        ...
        out_meta = {
            "molecule": metadata.get("molecule", "N2"),
        ...
        for figure, name in ((fig, "n2_polynomial_curve.png"),
                             (fig_err, "n2_polynomial_error.png"),
                             (fig_sweep, "n2_polynomial_degree_sweep.png")):
            path = here / name
