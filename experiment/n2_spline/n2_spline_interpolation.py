"""
n2_spline_interpolation.py -- reconstruct N2's dissociation curve from a
...
------------------------
The ``h2/``, ``h2o/``, ``beh2/``, ``o2/`` and ``n2/`` scripts all plot their
curves by joining neighbouring points with straight line segments -- that is
what matplotlib's ``plot`` does, and it is the crudest possible
reconstruction of a smooth function. It is fine for looking at. It is not
fine for *using*.

That matters for N2 for the same reason it matters for O2: the default scan
is nine points at 0.2 A spacing (see the sibling ``o2_spline/`` for the exact
argument). Nine samples that coarse barely resolve the bottom of the
potential well -- and N2's well is unusually *narrow*, since a triple bond is
much stiffer than a single or double one, so the true minimum is even easier
to miss between two coarse samples here than it is for O2. So:
...
oscillation a single high-degree polynomial suffers -- which is precisely
what the sibling ``n2_polynomial/`` folder demonstrates going wrong.
...
interpolant would have no data on one side at all, so the error would say
more about extrapolation than about the method.

OUTPUT
-------
  n2_spline_interp.json / .csv   per-node leave-one-out errors, plus summary
                                 metrics and the interpolated minimum in the
                                 metadata
  n2_spline_dense.csv            the smooth reconstructed curve, densely
                                 sampled, for your own plots
  n2_spline_curve.png            nodes, the spline through them, and the old
                                 straight-line version for comparison
  n2_spline_error.png            leave-one-out error per node, spline vs linear
...
experiment/n2/ already saved.
  ...
  DEFAULT -- finds n2_scan.json (or .csv) in experiment/n2/
    python experiment/n2_spline/n2_spline_interpolation.py

  A PARTICULAR SCAN -- .json preferred, .csv accepted
    python experiment/n2_spline/n2_spline_interpolation.py --scan n2_scan.json
    python experiment/n2_spline/n2_spline_interpolation.py --scan n2_scan.csv
    python experiment/n2_spline/n2_spline_interpolation.py --scan fine_scan.json

  WHICH ENERGY COLUMN -- 'vqe' by default. Interpolating 'exact' instead
  separates coarse-sampling error from VQE optimizer noise.
    python experiment/n2_spline/n2_spline_interpolation.py --column vqe
    python experiment/n2_spline/n2_spline_interpolation.py --column exact
    python experiment/n2_spline/n2_spline_interpolation.py --column hf
...
# N2's experimental equilibrium bond length, for comparison against whatever
# the interpolated minimum comes out as.
EQUILIBRIUM_BOND = 1.098   # Angstrom

# Where to look for a saved N2 scan when --scan isn't given.
DEFAULT_SCAN_NAMES = ["n2_scan.json", "n2_scan.csv"]


def find_scan(explicit: str | None, here: Path) -> Path:
    """Locate the saved N2 scan to interpolate.

    Checked in order: what the user asked for, then the standard names inside
    ``experiment/n2/``. The error message lists everywhere it looked and what
...
def main() -> None:
    ...
    parser.add_argument("--save", type=str, default=None, metavar="FILE",
                        help="where to save the results (default: n2_spline_interp.json/.csv "
                             "next to this script)")
    ...
    if len(x) < 4:
        raise SystemExit(
            ...
            f"least 4. Re-run the N2 scan with a finer --step."
        )
    ...
    ax.set_title("N$_2$ curve reconstructed by cubic spline from a coarse scan")
    ...
    # -- save -----------------------------------------------------------
    if not args.no_save:
        target = resolve_target(args.save, here, "n2_spline_interp")
        ...
        curve_png = here / "n2_spline_curve.png"
        error_png = here / "n2_spline_error.png"
