"""
o2_polynomial_interpolation.py -- reconstruct O2's dissociation curve from a
...
For O2 that failure mode is not academic. The default scan is nine points, so
the full interpolating polynomial is degree 8, and the region where the
oscillation is worst -- the ends -- is precisely where the interesting physics
lives: the repulsive wall on the left and the dissociation plateau on the
right. A physically meaningless wobble on the plateau can invent a spurious
second minimum that was never in the data.

Compare this against ``../o2_spline/``, which fits low-degree cubics
...
Fitting a degree-8 polynomial in raw Angstroms is numerically nasty, and
without that rescaling some of the oscillation you would see would be
floating-point noise rather than the genuine Runge effect this script is
trying to show. Getting that distinction right matters when the whole point is
to attribute the failure correctly.

OUTPUT
-------
  o2_polynomial_interp.json / .csv    per-node leave-one-out errors at the
                                      chosen degree, plus summary metrics
  o2_polynomial_degrees.csv           leave-one-out error for every degree
  o2_polynomial_dense.csv             the reconstructed curve, densely sampled
  o2_polynomial_curve.png             nodes, the polynomial, and linear for
                                      comparison
  o2_polynomial_error.png             per-node error, polynomial vs linear
  o2_polynomial_degree_sweep.png      error against degree -- the Runge story
...
experiment/o2/ already saved.
  ...
  DEFAULT -- finds o2_scan.json (or .csv) in experiment/o2/
    python experiment/o2_polynomial/o2_polynomial_interpolation.py

  A PARTICULAR SCAN -- .json preferred, .csv accepted
    python experiment/o2_polynomial/o2_polynomial_interpolation.py --scan o2_scan.json
    python experiment/o2_polynomial/o2_polynomial_interpolation.py --scan o2_scan.csv
    python experiment/o2_polynomial/o2_polynomial_interpolation.py --scan o2_singlet_scan.json
    python experiment/o2_polynomial/o2_polynomial_interpolation.py --scan o2_singlet_scan.csv
  ...
  9-point O2 scan that is ~19 mHa of sampling error against ~3 of noise.
    python experiment/o2_polynomial/o2_polynomial_interpolation.py --column vqe
    python experiment/o2_polynomial/o2_polynomial_interpolation.py --column exact
    python experiment/o2_polynomial/o2_polynomial_interpolation.py --column hf
  ...
  covers every degree that can be.
    python experiment/o2_polynomial/o2_polynomial_interpolation.py --degree 6
    python experiment/o2_polynomial/o2_polynomial_interpolation.py --degree 3
...
def find_scan(explicit: str | None, here: Path) -> Path:
    """Locate the saved O2 scan to interpolate. See the o2_spline twin."""
...
def main() -> None:
    ...
    parser.add_argument("--save", type=str, default=None, metavar="FILE",
                        help="where to save the results (default: "
                             "o2_polynomial_interp.json/.csv next to this script)")
    ...
    if n < 3:
        raise SystemExit(
            ...
            f"about polynomial degree. Re-run the O2 scan with a finer --step."
        )
    ...
    print("  compare with the cubic spline: python experiment/o2_spline/o2_spline_interpolation.py")
    print()
    ...
    ax.set_title(f"O$_2$ curve reconstructed by a degree-{degree} polynomial\n"
    ...
    # -- save -----------------------------------------------------------
    if not args.no_save:
        target = resolve_target(args.save, here, "o2_polynomial_interp")
        ...
        for figure, name in ((fig, "o2_polynomial_curve.png"),
                             (fig_err, "o2_polynomial_error.png"),
                             (fig_sweep, "o2_polynomial_degree_sweep.png")):
            path = here / name
